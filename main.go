...
func main() {
	...
	long := flag.Bool("long", false, "long is 24 words length, otherwise it'll be 12")
	pass := flag.String("pass", "", "protect bip39 mnemonic with a passphrase")
	number := flag.Int("n", 1, "set number of keys to generate")
	...
	km, err := NewKeyManager(bitsize, *pass, *mnemonic)
	if err != nil {
		log.Fatal(err)
	}
